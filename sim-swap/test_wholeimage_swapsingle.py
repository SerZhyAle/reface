...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        img = np.array(img_a_align_crop_pil, dtype=np.float)
        # img = img_mask * target_image + (1-img_mask) * img
        final_img = img.astype(np.uint8)
        cv2.imwrite(opt.pic_a_path+ "_", final_img)
        ...
        reverse2wholeimage(b_align_crop_tenor_list, swap_result_list, b_mat_list, crop_size, img_b_whole, logoclass, \
            os.path.join(opt.output_path, 'res_'+now.strftime("%Y%m%d%H%M%S")+'.jpg'), opt.no_simswaplogo,pasring_model =net,use_mask=opt.use_mask, norm = spNorm)
